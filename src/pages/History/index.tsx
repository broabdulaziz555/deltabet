...
const FILTERS = ['all', 'won', 'lost', 'real', 'demo'] as const
...
const History: React.FC = () => {
  ...
  const fetchBets = async (f = filter, p = 1) => {
    ...
    try {
      ...
      else if (f === 'real') params.mode = 'real'
      else if (f === 'demo') params.mode = 'demo'
                ...
                <div key={b.id} className="db-card p-3">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-lg ${b.mode === 'demo' ? 'bg-db-blue/10 text-db-blue' : 'bg-db-red/10 text-db-red'}`}>
                        {b.mode.toUpperCase()}
                      </span>
                    </div>
