...
const Deposit: React.FC = () => {
  ...
  useEffect(() => {
    txAPI.getCards()
      .then((r) => {
        setCards(r.data)
        if (r.data.length > 0) setSelectedCard(r.data[0])
      })
      .catch(() => toast.error(t('deposit.errors.noCard')))
    ...
    <MobileLayout>
      <div className="flex flex-col items-center justify-center min-h-full px-6 text-center gap-6">
        ...
        <div>
          ...
          <p className="text-db-text2 text-sm">Your request is being reviewed by admin. Balance will update upon approval.</p>
        </div>
        <button className="btn-bet px-8 py-3 text-white" style={{ background: 'linear-gradient(135deg,#3a86ff,#1565c0)' }} onClick={() => nav('/transactions')}>
          View Transactions
        </button>
      </div>
    </MobileLayout>
        ...
        <div className="flex-1 px-4 py-4 flex flex-col gap-4 pb-8">
          ...
          {loadingCards ? <Spinner /> : selectedCard ? (
            <div className="db-card p-4"
              ...
              <div className="flex items-center justify-between mb-2">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-white">{selectedCard.card_type.toUpperCase()}</span>
                    <span className={`text-xs px-2 py-0.5 rounded ${selectedCard.card_type === 'humo' ? 'bg-green-500/20 text-green-400' : 'bg-blue-500/20 text-blue-400'}`}>
              ...
              <div className="flex items-center justify-between bg-db-bg2 rounded-xl p-3">
                <span className="mono font-bold text-lg tracking-widest">
                  {selectedCard.card_number.replace(/(\d{4})/g, '$1 ').trim()}
                </span>
                <button onClick={copyCard} className="flex items-center gap-1.5 text-xs font-bold transition-colors"
            ...
            <div className="db-card p-4 text-center text-db-text2">{t('deposit.errors.noCard')}</div>
          )}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              <input
                type="number"
                className={`db-input text-right pr-14 ${amountError ? 'error' : ''}`}
                placeholder="10000"
                value={amount}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex gap-2">
              <input
                className={`db-input flex-1 ${promoError ? 'error' : promoInfo ? 'border-db-green/50' : ''}`}
          ...
          <div className="bg-db-blue/10 border border-db-blue/20 rounded-xl p-4 text-xs text-db-text2 space-y-1">
            ...
            <div>2. Transfer any amount (admin approves manually)</div>
            <div>3. Upload your transfer screenshot or PDF cheque</div>
            <div>4. Submit and wait for approval</div>
          </div>
          ...
          >
            {loading ? <div className="spinner mx-auto" /> : t('deposit.submitBtn')}
          </button>
        </div>
      </div>
    </MobileLayout>
